...
data = pickle.load(open("last_ivs_vs_field.pkl","rb"))
...
def find_kink(fb_ic_fixed, polarity):
    ...
    model, (kbest,a,b,kink_c), X2 = fit_kink_model(np.arange(a,b),
                                fb_ic_fixed[a:b])
    kink_ind = int(kbest)
    kink_ind += 2500 # hard coded correction... BAD
...
for row in range(2):
    ...
    for polarity in [1,-1]:
        ...
        labels = []
        try:
            for ind_fc in np.arange(0,n_fc,1):
                    v_fc = data.v_fcs[ind_fc]
                    siv = get_sampled_iv(ind_fc=ind_fc, row=row,polarity=polarity)
                    c  = cm(ind_fc)
                    if ind_fc%4 == 0:
                        c = "k"
                    plt.plot(siv.fb_ic_fixed[siv.sample_inds[-40]:siv.sample_inds[-1]:1], 
                             color=c, lw=ind_fc%3+1)
                    labels.append(f"v_fc={v_fc:0.2f}V")
        except:
            print(f"row={row} failed plotting")
