...
fb_offset = fba_offsets[column_number,0]
...
time.sleep(1)
set_volt(14000)
data = ec.getNewData(delaySeconds=0, minimumNumPoints=150000) # collecting the data over this changing bias point
...
fb = data[column_number,0, :, 1]
err = data[column_number,0, :, 0]

ups, downs = find_arl_resets_lookboth(fb, err, flux_jump_threshold_dac_units, fb_offset,
lookback_samples=20, lookback_threshold=2000)

blip_time_from_0_s = 1e-9*(ec._lastGetNewDataFirstTriggerUnixNano-time_nano_after_last_set_volt)
