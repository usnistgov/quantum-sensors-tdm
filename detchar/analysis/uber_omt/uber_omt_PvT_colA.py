...
def main():
    ...
    hwmapfile = 'hwmap.pkl'    
    fnames =[ 'uber_omt_ColumnA_ivs_20230519_1684529743.json'
            ]
    outfile = 'tsweep_analyzed_colA.pkl'        
    with open(os.path.join(hwmappath,hwmapfile),'rb') as opf:
        hwmap = pickle.load(opf)        
    outpath = os.path.join(path,'uber_omt_PvT_output_20230524')
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    results = {}
    for fname in fnames:
        df = IVTempSweepData.from_file(os.path.join(path,fname))
        # Cut all IVs with temps t_cut_high K and above
        t_cut_high = 0.2
