...
temps_mk = np.arange(45,85,5)
print(f"{temps_mk} mK")
sweep = temp_sweeper.get_sweep(
    dacs, set_temps_k=temps_mk * 1e-3, extra_info={"field coil current (Amps)": 0}
)
sweep.to_file("20210921_2_CX_NSLS_temp_sweep_IVs_with_zero_bias_track.json", overwrite=True)
sweep.plot_row(row=0)
sweep.data[0].plot()


plt.ion()
sdata = detchar.IVTempSweepData.from_file("20210921_2_CX_NSLS_temp_sweep_IVs_with_zero_bias_track.json")
