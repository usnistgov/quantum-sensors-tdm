...
class EasyClientDastard:
    ...
    def _connectRecordSub(self, verbose):
        ...
        if verbose:
            print(("Collecting records from dastard at %s" % self.recordSubAddress))
    ...
    def _getStatus(self):
        ...
        while True:
            ...
            if all(
                [self.messagesSeen[t] > 0 for t in ["STATUS", "LANCERO", "SIMPULSE"]]
            ):
                ...
                return
        raise Exception(
            f"didn't get source and status messages, messagesSeen: {self.messagesSeen}"
        )
    ...
    def setMix(self, mixFractions):
        ...
        if not numpy.all(numpy.shape(mixFractions) == (self.numColumns, self.numRows)):
            raise ValueError(
                "mixFractions should either a number or a list/array with (numColumns, numRows) elements"
            )
        config = {
            "ChannelIndices": np.arange(
                1, self.numColumns * self.numRows * 2, 2
            ).tolist(),
    ...
    def getNewData(
        ...
        while True:
            ...
            if n_observed == self.numChannels:
                ...
                k_complete = [
                    k
                    for k in keys
                    if counter[k - firstTriggerFramecount] == self.numChannels
                ]
        ...
        if sendMode != "raw":
            dataOut[:, :, :, 1] = (
                dataOut[:, :, :, 1] >> 2
            )  # ignore 2 lsbs (frame bit and trigger)
        if sendMode == 2:
            dataOut[:, :, :, 0] = (
                dataOut[:, :, :, 0] >> 2
            )  # ignore 2 lsbs if this is also a fb (just scaling)
        if toVolts:
...
if __name__ == "__main__":
    if False:
        ...
        for sendMode in sendModes:
            data = c.getNewData(
                delaySeconds=0.001, minimumNumPoints=6000, sendMode=sendMode
            )
            plt.figure()
