...
class externTrigClient(object):
    ...
    def __init__(self, host=None, port=None):
        self.client = nasa_client.Client(host=host, port=port)
    ...
    def open_file(self, filename):
        ...
        if dataset_exists:
            ttimes = fp['trig_times']
            assert ttimes.attrs['Nrows'] == self.client.nrow
            assert ttimes.attrs['sample_rate_hz'] == float(self.client.sample_rate)
        else:
            ttimes = fp.create_dataset('trig_times', (128,), dtype=np.uint64, maxshape=(None,),
                                       chunks=True, compression='gzip')
            ttimes.attrs['Nrows'] = self.client.nrow
            ttimes.attrs['Ntrigs'] = 0
            ttimes.attrs['Ncols'] = self.client.ncol
            ttimes.attrs['lsync'] = self.client.lsync
            ttimes.attrs['sample_rate_hz'] = float(self.client.sample_rate)
    ...
    def close_file(self):
        if self.output_file is not None:
            ...
            ttimes = self.output_file['trig_times']
            nt = ttimes.attrs['Ntrigs']
            ttimes.resize((nt,))
    ...
    def get_data_packets(self):
        ...
        for data, hdr in zip(payloads, headers):
            ...
            if hdr['packet_version'] <= 9:
                tcount = hdr['time_count_of_last_sample'] - hdr['record_samples'] + 1
            else:
                tcount = hdr['frame_count_of_last_sample'] - hdr['record_samples'] + 1
            assert cnum < self.n_etrig_chan
            self.chan_queues[cnum].append((data,tcount))
        return len(payloads)
    ...
    def process_data_packets(self):
        ...
        while True:
            ...
            self.last_packet_frame = tstamp
            self.lastTrigValue = bitvector.pop()
            packet_ttimes = transitions + (tstamp*self.nrow)

            if self.output_file is not None:
                ttimes = self.output_file['trig_times']

                # Resize the HDF5 dataset, if needed
                first = ttimes.attrs['Ntrigs']
                end = first + len(transitions)
                maxtrigs = int(ttimes.size)
                if end > maxtrigs:
                    maxtrigs = 128*((end+128)/128)
                    ttimes.resize((maxtrigs, ))

                ttimes[first:end] = packet_ttimes
                ttimes.attrs['Ntrigs'] = end

            if self.binary_file is not None:
                self.binary_file.write(packet_ttimes.tostring(order="C"))
