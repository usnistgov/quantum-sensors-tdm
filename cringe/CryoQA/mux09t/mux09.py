...
class MUX09:
    ...
    def row_bias_sweeper(self, row):
        ...
        for sweep_point in range(self.number_steps):
            ...
            elif have_icmin is True:
                sample0 = self.row_average_adcpp_sweep[row, sweep_point] <= \
                          self.row_average_adcpp_sweep[row, sweep_point-1]
                sample1 = self.row_average_adcpp_sweep[row, sweep_point-1] <= \
                          self.row_average_adcpp_sweep[row, sweep_point-2]
                sample2 = self.row_average_adcpp_sweep[row, sweep_point - 2] <= \
                          self.row_average_adcpp_sweep[row, sweep_point - 3]
                if sample0 and sample1 and sample2:
                    self.icmax[row] = self.row_sweep_dac_values[np.argmax(self.row_average_adcpp_sweep[row, :])]
                    have_icmax = True
                    icmax_sweep_point = np.argmax(self.row_average_adcpp_sweep[row, :])

                if have_icmax and (sweep_point >= (self.num_points_after_max + icmax_sweep_point)):
                    break
    ...
    def load_npz(self, file_name):
        ...
        self.baselines          = loaded_file['baselines']
