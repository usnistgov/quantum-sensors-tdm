...
# 		self.tri_wvfm_widget.setFixedWidth(self.dfbx2_widget1.width()/2+10)
	...
	def LED_changed(self):
		...
		print("SCREAM LED boolean (True = OFF):", self.LED, tc.ENDC)
		if self.LED ==1:
...
#		 if self.unlocked == 1:
	...
	def status_changed(self):
		...
		print("SCREAM ST boolean:", self.ST, tc.ENDC)
		if self.ST ==1:
	...
	def send_global(self, parameter, value):
		if parameter == "PS":
			self.PS = value
			GPI = 8
			print("SCREAM PS:", value, tc.ENDC)
		if parameter == "XPT":
			self.XPT = value
			GPI = 9
			print("SCREAM XPT:", value, tc.ENDC)
		if parameter == "TP":
			self.TP = value
			self.TPboolean = 0
			if value != 0:
				self.TPboolean = 1
				self.decode_tp()
				print("SCREAM Test Pattern Hi Byte:", hex(self.hibytes), tc.ENDC)
				GPI = 12
				self.send_cmd(GPI, self.hibytes)
				print("SCREAM Test Pattern Lo Byte:", hex(self.lobytes), tc.ENDC)
				GPI = 13
				self.send_cmd(GPI, self.lobytes)
			GPI = 11
			value = self.TPboolean
			print("SCREAM Test Pattern Boolean:", self.TPboolean, tc.ENDC)
		if parameter == "NSAMP":
			self.NSAMP = value
			GPI = 40
			print("SCREAM NSAMP:", value, tc.ENDC)
		if parameter == "SETT":
			self.SETT = value
			GPI = 41
			print("SCREAM SETT:", value, tc.ENDC)
		if parameter == "CARD":
			self.card_delay = value
			GPI = 42
			print("SCREAM CARD_DELAY:", value, tc.ENDC)
		if parameter == "PROP":
			self.prop_delay = value
			GPI = 43
			print("SCREAM PROP_DELAY:", value, tc.ENDC)
	...
	def send_ARL(self, parameter, value):
		if parameter == "ARLsense":
			self.ARLsense = value
			GPI = 16
			print("SCREAM ARLsense:", value, tc.ENDC)
		if parameter == "RLDpos":
			self.RLDpos = value
			GPI = 17
			print("SCREAM RLDpos:", value, tc.ENDC)
		if parameter == "RLDneg":
			self.RLDneg = value
			GPI = 18
			print("SCREAM RLDneg:", value, tc.ENDC)
		self.send_cmd(GPI, value)
